...
auto Optimizer::OptimizeNLJAsHashJoin(const AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef {
  ...
  // std::cout << "plan: " << plan->ToString() << std::endl << std::endl;
  // std::cout << "plan_schema: " << plan->OutputSchema().ToString() << std::endl << std::endl;
  ...
  if (optimized_plan->GetType() == PlanType::NestedLoopJoin) {
    ...
    // std::cout << "plan: " << plan->ToString() << std::endl << std::endl;
    // std::cout << "nlj_plan: " << nlj_plan.ToString() << std::endl << std::endl;
    // std::cout << "nlj_plan.output_schema: " << nlj_plan.output_schema_->ToString() << std::endl << std::endl;
    // std::cout << "Predicate of the nlj_plan: " << nlj_plan.Predicate()->ToString() << std::endl << std::endl; // e.g.
    // (#0.0=#1.0)

    // 这个写法很妙
    if (const auto *expr = dynamic_cast<const ComparisonExpression *>(nlj_plan.Predicate().get());
        expr != nullptr) {  // <column expr> = <column expr>
      ...
      if (expr->logic_type_ == LogicType::And) {
        if (const auto *left_expr = dynamic_cast<const ComparisonExpression *>(expr->children_[0].get());
          ...
          if (const auto *right_expr = dynamic_cast<const ComparisonExpression *>(expr->children_[1].get());
              right_expr != nullptr) {
            // std::cout << "left_expr: " << left_expr->ToString() << std::endl << std::endl;
            // std::cout << "right_expr: " << right_expr->ToString() << std::endl << std::endl;
