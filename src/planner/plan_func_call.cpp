...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (args.size() != 1) {
    throw Exception(fmt::format("the number of args is {} which is not supported", args.size()));
  }
  // 3. return a `StringExpression` std::shared_ptr.
  return func_name == "lower" ? std::make_shared<StringExpression>(args[0], StringExpressionType::Lower)
                              : std::make_shared<StringExpression>(args[0], StringExpressionType::Upper);
}
