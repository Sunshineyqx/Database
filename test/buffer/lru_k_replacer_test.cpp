...
TEST(LRUKReplacerTest, ConcurrencyTest) {  // NOLINT
  ...
  for (int i = 0; i < 1000; i += 100) {
    threads.emplace_back(record_access_thread, i, i + 100, true, true);
  }
  ...
  threads.emplace_back(record_access_thread, 250, 1000, true, true);
  threads.emplace_back(record_access_thread, 500, 1000, true, true);
  threads.emplace_back(record_access_thread, 750, 1000, true, true);
  threads.emplace_back(remove_task_thread, 250, 400);
  threads.emplace_back(remove_task_thread, 400, 500);
  threads.emplace_back(set_evictable_thread, 0, 150, false);
  threads.emplace_back(set_evictable_thread, 150, 250, false);
  ...
  threads.emplace_back(remove_task_thread, 250, 400);
  threads.emplace_back(remove_task_thread, 400, 500);
  threads.emplace_back(record_access_thread, 500, 700, false, true);
  threads.emplace_back(record_access_thread, 700, 1000, false, true);
  threads.emplace_back(evict_task_thread, 500, 600, true);
  threads.emplace_back(evict_task_thread, 600, 800, true);
  threads.emplace_back(evict_task_thread, 800, 1000, true);
